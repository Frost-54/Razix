...
rz_critical_section rz_critical_section_create_ex(u32 spintCount)
{
    ...
    InitializeSRWLock((PSRWLOCK) &cs.m_Internal.buffer);
...
{
    AcquireSRWLockExclusive((PSRWLOCK) &cs->m_Internal.buffer);
}

bool rz_critical_section_try_lock(rz_critical_section* cs)
{
    return TryAcquireSRWLockExclusive((PSRWLOCK) &cs->m_Internal.buffer) != 0;
}

void rz_critical_section_unlock(rz_critical_section* cs)
{
    ReleaseSRWLockExclusive((PSRWLOCK) &cs->m_Internal.buffer);
}
